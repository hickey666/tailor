...
class StringHelper extends \yii\helpers\StringHelper{
    ...
    public static function formatUrlsInText($str){
        ...
        preg_match_all('/https?:\/\/(([a-zA-Z0-9_-])+(\.)?)*(:\d+)?(\/((\.)?(\?)?=?&?[a-zA-Z0-9_-](\?)?)*)*/i',$str,$arr);
        if(!$arr[0])
        {
            return $str;
        }
        ...
        foreach ($arr[0] as $v)
        {
            $tmp = explode($v,$str);
            $_tmp[]=$tmp[0];
            $_tmp[]="<a href='{$v}' target='_blank' style='color:#4d9fef' title='网页链接'>网页链接</a>";
            $_tmp[]=$tmp[1];
     ...
     * @param string $keyStart 子字符串的前一个字符
     * @param string $keyEnd   子字符串的后一个字符
     * @param bool   $case     是否区分大小写，默认区分
     *
     * @return bool|string 字符串为空或子字符参数错误时返回false，查找成功返回子字符串
     */
    public static function getSubBetween($str, $keyStart, $keyEnd, $case = false)
    {
        if (empty($str)) {
            return false;
        }
        ...
        if (empty($keyStart)) {
            ...
            $posStart = mb_stripos($str, $keyStart) + 1;
        } else {
            $posStart = mb_strpos($str, $keyStart) + 1;
        }
        if ($case) {
            $posEnd = mb_stripos($str, $keyEnd);
        } else {
            $posEnd = mb_strpos($str, $keyEnd);
        }

        if ($posStart === false || $posEnd === false || $posEnd < $posStart) {
            return false;
        }
        return self::substr($str, $posStart, $posEnd - $posStart);
    }

    /**
     * 返回字符串中的所有单词
     * @param $str
     * @param bool $distinct 为true时，去重
     * @return array
     */
    public static function split_en_str($str, $distinct = true) {
