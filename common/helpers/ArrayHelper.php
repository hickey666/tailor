...
class ArrayHelper extends \yii\helpers\ArrayHelper
{
     ...
     * ---------------------------------------------------
     */
    public static function tree_to_list_no_order($tree, $child = '_child', &$list = []){
        if(is_array($tree)) {
            $refer = [];
            foreach ($tree as $key => $value) {
                $reffer = $value;
                if(isset($reffer[$child])){
                    unset($reffer[$child]);
                    static::tree_to_list_no_order($value[$child], $child, $list);
                }
                $list[] = $reffer;
            }
        }
     ...
     *
     * @param array $array
     * @param string $chosen 选中的key，默认为id
     * @return array
     * @throws \Error
     */
    public static function parseIndex(array $array, $chosen = 'id')
    {
        if (!is_array($array)) {
    ...
    /**
     * 简化获得的数据   [0=>['id'=>1,'name'=>'a],1=>['id'=2,'name'=>'b']]  ======>[1=>'a',2=>'b']
     *
     * @param array $data
     * @param str $id 需要当做键名的参数名
     * @param str $param 需要作为键值的参数名
     * @return array 结果集
     */
    public static function simplify($data,$id='id',$param='name')
    {
        $result = [];
        foreach($data as $key=>$value){
            $result[$value[$id]] = $value[$param];
        }
        return $result;
    }
     ...
     * @param string|array $keys  需要删除的值
     *
     * @return array 删除后的数组
     */
    public static function unsetKey($array, $keys)
    {
        if (!is_array($array)) {
            return [];
        }
        if (is_array($keys)) {
            ...
            unset($array[array_search($keys, $array)]);
            return $array;
        }
    }
}
